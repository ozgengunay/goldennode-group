...
public class QueueServiceImpl<E> implements QueueService<E> {
    ...
    public boolean remove(String queueId, Object object) throws GoldenNodeException {
        try {
            Response response = RestClient.call("/goldennode/queue/id/{queueId}/remove/object/{object}".replace("{queueId}", queueId).replace("{object}", Utils.encode(Utils.encapObject(object))), "DELETE");
            if (response.getStatusCode() == 200)
                return Boolean.parseBoolean(response.getEntityValue());
            else {
                throw new GoldenNodeException("Error occured" + response.getStatusCode() + " - " + response.getEntityValue());
    ...
    public boolean add(String queueId, E element) throws GoldenNodeException {
        try {
            Response response = RestClient.call("/goldennode/queue/id/{queueId}/add/element/{element}".replace("{queueId}", queueId).replace("{element}", Utils.encode(Utils.encapObject(element))), "POST");
            if (response.getStatusCode() == 200)
                return Boolean.parseBoolean(response.getEntityValue());
            else {
                throw new GoldenNodeException("Error occured" + response.getStatusCode() + " - " + response.getEntityValue());
    ...
    public boolean offer(String queueId, E element) throws GoldenNodeException {
        try {
            Response response = RestClient.call("/goldennode/queue/id/{queueId}/offer/element/{element}".replace("{queueId}", queueId).replace("{element}", Utils.encode(Utils.encapObject(element))), "POST");
            if (response.getStatusCode() == 200)
                return Boolean.parseBoolean(response.getEntityValue());
            else {
                throw new GoldenNodeException("Error occured" + response.getStatusCode() + " - " + response.getEntityValue());
